...
ArrayPointerTest::ArrayPointerTest(int count) {
  ...
  cout << "size of a Duck object: " << sizeof(ducks3_) << endl;
  ...
  cout << "size of a Duck object: " << sizeof(ducks4_) << endl;
}
...
void ArrayPointerTest::NameTheDucks(int array_number) {
  ...
  switch(array_number) {
    ...
    case 3:
      ...
      for (int i=0; i<count_; i++) {
         ducks3_[i][0].set_name(names[i]);
         ducks3_[i][0].set_number(3);
      }
      break;
