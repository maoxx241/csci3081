...
void Bus::Tocsv(std::ostream& out) {
  out << "Name," << name_ << ",Type,";
  ...
  out << ",Passengers,(0),Total,Num,of,Passengers,"<<
    total_passenger << std::endl;
}
...
void Bus::SetColor() {
  int alp = std::min(255, std::max(75, static_cast<int>(
      (1-bus_data_.num_passengers/bus_data_.capacity*5)*225)));

  if (outgoing_route_->IsAtEnd()) {
    Color color_ = {128, 0, 0, alp};
    bus_data_.color = color_;
    return;
  }
  Color color_ = {255, 215, 0, alp};
  bus_data_.color = color_;
}
